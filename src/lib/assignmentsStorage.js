...
export function listSavedSubstitutionDatesAscending() {
  return loadAllAssignments()
    .map((a) => a.date)
    .filter(Boolean)
...
/** Previous or next saved substitution date relative to current (chronological). */
export function adjacentSavedSubstitutionDate(currentISO, direction) {
  const dates = listSavedSubstitutionDatesAscending();
  ...
  if (idx === -1) {
    if (direction === "next") {
      return dates.find((d) => d > currentISO) ?? null;
    }
    const before = dates.filter((d) => d < currentISO);
...
export function saveAssignmentsForDate(
  dateISO,
  rows,
  { leaveTeachers = [], excludedTeachers = [] } = {}
) {
...
export function loadAssignmentsForDate(dateISO) {
  ...
  return {
    date: entry.date,
    leaveTeachers: entry.leaveTeachers || [],
    excludedTeachers: entry.excludedTeachers || [],
    rows: entry.rows || [],
