...
import { buildSubjectShortMap, loadSchoolStorageRaw, toSubjectShortName } from "./settingsStorage";
...
function getLessonCell(timetable, kind, itemContext, day, lessonPeriod, shortMap) {
  ...
  if (kind === "class") {
    ...
    if (!card) return null;
    return {
      subject: toSubjectShortName(card.subject, shortMap),
  ...
  return {
    subject: toSubjectShortName(lesson.subject, shortMap),
