...
export default function SubjectTimeOffModal({ open, subject, onClose, onSave }) {
  ...
  if (!open || !subject || !dims) return null;
  ...
  return (
    <div className="modal-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-labelledby="subject-timeoff-title"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 id="subject-timeoff-title" className="modal-title">
          Time off — {subject.name}
        </h3>
        <p className="card-desc">
          Set when this subject may be taught (lesson periods only; breaks are not shown). By default,
          slots from Monday period 1 up to the school&apos;s{" "}
          <strong>{periodsPerWeek} periods per week</strong> limit are available (✓); remaining grid
          slots are unavailable (✗). Tick a slot (✓) to allow this subject in that period when
          generating the timetable. Click Save to store this subject&apos;s time off.
        </p>
