...
import GenerateTimetableBusy from "../components/GenerateTimetableBusy";
...
function classDisplayLabel(cls) {
  return String(cls?.title || cls?.label || "").trim();
}

function findTimetableIndex(items, query, getLabel) {
  const q = String(query || "").trim().toLowerCase();
  if (!q || !items.length) return -1;
  const entries = items.map((item, index) => ({
    index,
    label: String(getLabel(item) || "").trim().toLowerCase(),
  }));
  const exact = entries.find((e) => e.label === q);
  if (exact) return exact.index;
  const starts = entries.find((e) => e.label.startsWith(q));
  if (starts) return starts.index;
  const partial = entries.find((e) => e.label.includes(q));
...
export default function TimetablePage() {
  ...
  const gotoClassTimetable = () => {
    const index = findTimetableIndex(classes, classSearchQuery, classDisplayLabel);
    if (index < 0) {
      setWarning(`No class matching "${classSearchQuery.trim()}".`);
      return;
    }
    setWarning("");
    setDraggingCardId(null);
  ...
  const gotoTeacherTimetable = () => {
    const index = findTimetableIndex(teachers, teacherSearchQuery, (name) => name);
    if (index < 0) {
      setWarning(`No teacher matching "${teacherSearchQuery.trim()}".`);
      return;
    }
    setWarning("");
    setDraggingCardId(null);
  ...
  return (
    <div className="page-content tt-page">
      {generating && <GenerateTimetableBusy progress={generationProgress} />}
              ...
              <div className="tt-class-view__main">
                <header className="tt-class-header">
                  ...
                  <div className="tt-goto-row">
                    <input
                      type="search"
                      className="tt-goto-row__input"
                      list="tt-class-goto-list"
                      ...
                      aria-label="Search class timetable"
                    />
                    <datalist id="tt-class-goto-list">
                      {classes.map((cls) => (
                        <option key={cls.id} value={classDisplayLabel(cls)} />
                      ))}
                    </datalist>
                    <button type="button" className="btn btn-outline btn--sm tt-goto-row__btn" onClick={gotoClassTimetable}>
                      Go
                    </button>
                  </div>
                </header>
                  ...
                  <div className="tt-goto-row">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          gotoTeacherTimetable();
                        }
                      }}
                      aria-label="Search teacher timetable"
                    />
                    <datalist id="tt-teacher-goto-list">
                      {teachers.map((name) => (
                        <option key={name} value={name} />
                      ))}
                    </datalist>
                    <button type="button" className="btn btn-outline btn--sm tt-goto-row__btn" onClick={gotoTeacherTimetable}>
                      Go
                    </button>
                  </div>
                </header>
