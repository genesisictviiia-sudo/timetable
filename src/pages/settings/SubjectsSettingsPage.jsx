...
import SubjectTimeOffModal from "../../components/SubjectTimeOffModal";
import {
  downloadCsvFile,
  parseCsv,
...
export default function SubjectsSettingsPage() {
  ...
  const [timeOffSubjectId, setTimeOffSubjectId] = useState(null);
  ...
  const timeOffSubject = rows.find((r) => r.id === timeOffSubjectId) ?? null;
  ...
  const save = () => {
    ...
    const payload = filled.map((r) => ({
      ...
      timeOffGrid: r.timeOffGrid ?? null,
    }));
    saveSubjectsList(payload);
    alert("Subjects database saved.");
  };

  const saveTimeOffGrid = (subjectId, timeOffGrid) => {
    setRows((prev) => prev.map((r) => (r.id === subjectId ? { ...r, timeOffGrid } : r)));
  };
  ...
  return (
    <section className="card settings-panel-compact">
      ...
      <p className="card-desc">
        Name and shortcut for each subject. Use <strong>time off</strong> to set when each subject
        may be scheduled.
      </p>
      ...
      <SubjectTimeOffModal
        open={Boolean(timeOffSubjectId)}
        subject={timeOffSubject}
        onClose={() => setTimeOffSubjectId(null)}
